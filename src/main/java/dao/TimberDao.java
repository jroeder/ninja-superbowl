...
public class TimberDao extends AbstractDao {
	 ...
	 *
	 * @param id
	 *            the {@code GeoRegion} unique technical identifier
	 * @return a persisted {@code Timber} as a data transfer object
	 */
	@UnitOfWork
	public Integer getTimberMaxIndexByGeoRegionId(Long id) {

		EntityManager entityManager = entityManagerProvider.get();

		TypedQuery<Integer> typedQuery = entityManager.createQuery(
				"select max(t.index) from Timber t where t.geoRegion.id = :id", Integer.class);

		Integer timberIndex = (Integer) typedQuery.setParameter("id", id).getSingleResult();
